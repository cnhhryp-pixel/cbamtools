...
export default function PayPalCheckoutCard(){
 ...
 const reportHref=hasAssessment?"/report?"+q.toString():"/report";
...
`);
 ...
 async function copyRecipient(){
   try{await navigator.clipboard.writeText(PAYMENT_EMAIL);setCopied(true);window.setTimeout(()=>setCopied(false),1800)}
